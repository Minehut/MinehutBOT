...
module.exports = {
    run: async (client, msg, args) => {
        if (!args[0]) return msg.channel.send(':x: You must mention who you want to warn.');
        let user;
        ...
        if (!reason) return msg.channel.send(`:x: You must give a reason for why you want to warn ${user.tag}.`);
        ...
        client.db.table('punishments').insert({ id: id, punished: { name: user.tag, id: user.id, avatarURL: user.avatarURL }, moderator: { name: msg.author.tag, id: msg.author.id }, reason: reason, type: 'WARN', date: datepunished, active: false }).run();
