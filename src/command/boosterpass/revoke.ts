...
export default class BoosterPassGiveCommand extends MinehutCommand {
    ...
    constructor() {
        super('boosterpass-revoke', {
            ...
            channel: 'guild',
            description: {
                content: 'Remove a booster pass from a member, if you have given them one',
                usage: '<member>',
                examples: [
    ...
    async exec(
        ...
        if (!boosterPass)
            return msg.channel.send(`${process.env.EMOJI_CROSS} You haven't given this user a booster pass!`);
        ...
        return msg.channel.send(`${process.env.EMOJI_CHECK} Successfully removed a booster pass from **${member.user.tag}**`);
    }
