...
export default class BoosterPassGiveCommand extends MinehutCommand {
            ...
            args: [
                {
                    ...
                    type: 'member',
                    prompt: {
                        start: (msg: Message) => `${msg.author}, who should I give the booster pass too?`,
    ...
    async exec(
        ...
        if (boosterPasses.length == 2)
            return msg.channel.send(`${process.env.EMOJI_CROSS} You already have given two members booster passes!`);
