...
export default class BoosterInfoCommand extends MinehutCommand {
	...
	async exec(msg: Message, { member }: { member: GuildMember }) {
		...
		const mappedReceivedBoosterPasses = receivedBoosterPasses.map(
			bp => `• <@${bp.granterId}>`
		);

		if (boosterPasses.length <= 0 && receivedBoosterPasses.length <= 0)
			return msg.channel.send(
				`${process.env.EMOJI_CROSS} This member doesn't have any booster passes nor have they given any!`
			);
		...
		const embed = new MessageEmbed()
			...
			.addField(
				'Received From',
				mappedReceivedBoosterPasses.length > 0
					? mappedReceivedBoosterPasses.join('\n')
					: 'None',
				true
			)
			.addField(
				'Given To',
				mappedBoosterPasses.length > 0
					? mappedBoosterPasses.join('\n')
					: 'None',
				true
			)
			.setColor('BLURPLE');
