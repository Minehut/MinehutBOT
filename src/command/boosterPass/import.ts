...
export default class BoosterPassImportCommand extends MinehutCommand {
	...
	async exec(msg: Message) {
		...
		if (msg.attachments.size < 1)
			return msg.channel.send(
				`${process.env.EMOJI_CROSS} You must attach one file!`
			);
		...
		if (!attachment.name?.endsWith('.xlsx'))
			return msg.channel.send(
				`${process.env.EMOJI_CROSS} You must attach a XLSX file!`
			);
		...
		return msg.channel.send(
			`${process.env.EMOJI_CHECK} imported booster passes`
		);
	}
}
