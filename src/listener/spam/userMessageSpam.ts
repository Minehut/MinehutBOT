...
import { ONE_HOUR_MS } from '../../util/constants';
...
export default class UserMessageSpam extends Listener {
	...
	async exec(msg: Message) {
		if (
			...
			return;
		let memberMessageInfluxManager:
			| InfluxManager<Message>
			| undefined = this.client.influxManagerStore.getInfluxManager(
			msg.author.id
		);
		if (!memberMessageInfluxManager) {
			...
			this.client.influxManagerStore.registerInfluxManager(
				msg.author.id,
				memberMessageInfluxManager
			);
		}
