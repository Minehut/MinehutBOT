...
module.exports = {
	run: async (client, msg) => {
		...
		if (
			...
			msg.channel.id !== '676533595648688149' &&
			client.elevation(msg) < 2
		) {
			// Chat filter
			...
			if (matches.length > 0) {
				...
				try {
					msg.author.send(`
							Your message was deleted because it violated our automated chat filter. If you believe this is a mistake, please use the !meta command in the Minehut Discord and tell us about the issue.\n\n\`${msg.content.trim()}\``);
				} catch (err) {}
				switch (match.type.toLowerCase()) {
					case 'swear':
						let swearFeedback = await msg.channel.send(
							`${msg.author}, please do not swear on the Minehut discord. Thanks! ^-^`
						);
						setTimeout(() => swearFeedback.delete(), 5000);
						break;
					case 'spam':
						let spamFeedback = await msg.channel.send(
							`${msg.author}, please do not spam on the Minehut discord. Thanks! ^-^`
						);
						setTimeout(() => spamFeedback.delete(), 5000);
						break;
				}
				return;
			}
						...
						files.forEach(async file => {
							...
							if (meta.aliases.includes(command)) {
								if (msg.guild.id === client.config.staffguild) {
									client.log(
										`\`[STAFF]\` :wrench: ${msg.author.tag} (\`${msg.author.id}\`) used command in **#${msg.channel.name}** \`${msg.content}\``
									);
								} else {
									client.log(
										`:wrench: ${msg.author.tag} (\`${msg.author.id}\`) used command in **#${msg.channel.name}** \`${msg.content}\``
									);
								}
		...
		if (msg.content.includes('discord.gg')) {
			...
			if (!invwl.invites.includes(invite.guild.id)) {
				msg.delete();
				client.log(
					`:no_entry_sign: censored message by ${msg.author.tag} (\`${msg.author.id}\`) in ${msg.channel.name} (\`${msg.channel.id}\`) invite \`${invite.code}\` to ${invite.guild.name}: \n${msg.content}`
				);
			} else return;
		}
		...
		if (msg.mentions.users.size >= 10) {
			...
			client.log(
				`:helmet_with_cross: ${msg.author.tag} (\`${msg.author.id}\`) violated MAX_MENTIONS (<#${msg.channel.id}>): Too Many Mentions (${msg.mentions.users.size})`
			);
			client.db
				.table('punishments')
				.insert({
					id: id,
					punished: { name: user.tag, id: user.id, avatarURL: user.avatarURL },
					moderator: { name: client.user.tag, id: client.user.id },
					reason: reason,
					type: 'MUTE',
					date: datepunished,
					dateExpired: expDate,
