...
module.exports = {
    run: async (client, msgb, msga) => {
        if (msgb.embeds.length == 0 && msgb.content != msga.content && msga.channel.id && !msga.author.bot) {
					if (msga.guild.id === client.config.guildid && (msga.channel.id !== '585753135742582786') && msga.channel.id !== '676533595648688149') {
						// Chat filter
						...
						if (matches.length > 0) {
							...
							switch (match.type.toLowerCase()) {
								case 'swear':
									msga.channel.send(`${msga.author}, please do not swear on the Minehut discord. Thanks! ^_^`);
									break;
								case 'spam':
									msga.channel.send(`${msga.author}, please do not spam on the Minehut discord. Thanks! ^_^`);
									break;
							}
							return;
						}
